...
cpdef set_xint(int ising):
    ...
    *Modified*
    ...
    cdef int index, nx, nhalf, i, k, k0

    cdef double pin, si, ck, rk, t
...
cpdef sft(np.float64_t [:] x, np.float64_t om):
    """
    ...
    **Modified**
    ...
    cdef double pi = M_PI
    cdef double tp = 2.0*pi

    cdef int np1 = n+1
    cdef int l   = int(np.floor(6.0*om/tp))
    cdef double s   = np.sin(om)
    cdef double a   = 0.0
    cdef double b
    cdef double c   = 0.0
    cdef double d   = 0.0
